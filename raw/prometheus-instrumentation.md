...
For example, rather than separate metrics for different HTTP status codes, create a single metric with a label for the HTTP response code. You can then process the entire metric as one in rules and graphs.
...
To give perspective on underlying costs: node_exporter exposes metrics for every mounted filesystem. With 10,000 nodes, you could have roughly 100,000 time series for filesystem availability, which Prometheus can handle. However, adding quota per user across 10,000 users on 10,000 nodes would reach millions of time series, which exceeds current Prometheus capabilities.
...
A Java counter takes approximately 12-17 nanoseconds to increment depending on contention. Other languages will have similar performance. If that amount of time is significant for your inner loop, limit the number of metrics you increment in the inner loop and avoid labels (or cache the result of the label lookup) where possible.
