...
### Steps to deploy the Nginx profile:
...
3. Run a container with the profile in detached mode:
...
5. Test the profile with operations that should be restricted:
...
You can use `dmesg` to debug problems and `aa-status` to check the loaded profiles.

### Use dmesg

AppArmor sends verbose messaging to `dmesg`. An AppArmor line typically looks like:

```
[ 5442.864673] audit: type=1400 audit(1453830992.845:37): apparmor="ALLOWED" operation="open" profile="/usr/bin/docker" name="/home/jessie/docker/man/man1/docker-attach.1" pid=10923 comm="docker" requested_mask="r" denied_mask="r" fsuid=1000 ouid=0
```

In this example, `profile=/usr/bin/docker` indicates the Docker Engine daemon profile is loaded.
...
This shows the `docker-default` profile denying a ptrace operation, which is the expected behavior for unprivileged containers.

### Use aa-status

Check which profiles are loaded with:
