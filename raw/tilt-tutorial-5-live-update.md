---
source_url: https://docs.tilt.dev/tutorial/5-live-update.html
fetched: 2026-07-01
---
...
The destination path (`/app/api/`) is the absolute path *inside the container* where we want the files copied to.

> 💁‍♀️ Files you sync to the container *must* match paths that Tilt is already watching for the image configuration
...
For details, refer to the full [Live Update Reference](/live_update_reference.html#restarting-your-process).
...
The first argument is a command to run *inside the container*.
...
Now that you're familiar with how Tilt works and have seen some `Tiltfile` snippets, you're ready to follow the [Write a Tiltfile Guide](/tiltfile_authoring.html) and start using Tilt in your *own* project!
