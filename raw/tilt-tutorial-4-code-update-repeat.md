---
source_url: https://docs.tilt.dev/tutorial/4-code-update-repeat.html
fetched: 2026-07-01
---
...
We mentioned that Tilt embraces the concept of a [control loop](./2-tilt-up.html#the-control-loop), so once you've run `tilt up`, it's a "hands free" development experience.
...
* **`only` (optional): filters paths included in build context and restricts file watching to the subset of paths**
  ...
  Since `./web/vite.config.js` *is* under `./web/`, it hasn't been excluded, which is what we want!
...
> With an immutable tag, you're guaranteed *exactly* what got built is what will run.
>
...
The [Tilt Avatars](https://github.com/tilt-dev/tilt-avatars) project is configured to use Live Update for regular development, so we purposefully made a change in a config file that meant the full container would be rebuilt.
