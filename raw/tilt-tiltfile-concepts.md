---
source_url: https://docs.tilt.dev/tiltfile_concepts.html
fetched: 2026-07-01
---
...
indicating how to *find* config data (i.e. a file path, though in future this category may include other things).

For functions that can accept both types of strings – e.g. `k8s_yaml`, which can accept either path(s)
to YAML files or the YAML itself – we make use of typing to indicate whether a given string is *data* or
a *pointer to data*.
...
in the most common case, they contain *data*; e.g. `read_file('config.yaml')` or `local('generate_yaml.sh')` both
return YAML.
...
When specifying the `objects` param to `k8s_resource`, you can specify Kubernetes objects via a *Kubernetes object selector*.
...
2. This is currently just for the `objects` param. The workload arg is an *identifier*, not a *selector*.
...
We call this the object's *fullname*.
...
An object selector is only valid if *uniquely specifies a single object*; that
...
You may always use a *more* qualified object selector, even if a shorter one would
...
![Tilt's web UI shows a list of resources grouped by labels](/assets/img/resource-groups-expanded.png)
