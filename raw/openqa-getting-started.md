---
source_url: https://github.com/os-autoinst/openQA/blob/master/docs/GettingStarted.md
fetched: 2026-06-23
---
...
several components that are hosted in separate repositories.

The heart of the test engine is a standalone application called 'os-autoinst'.
In each execution, this application creates a virtual machine and uses
it to run a set of test scripts. 'os-autoinst' generates a video,
screenshots and a JSON file with detailed results.

'openQA' on the other hand provides a web based user interface and
...
_test group_, which itself is formed by the top-folder of the test module
...
**distri**
: a test distribution but also sometimes referring to a _product_, composed of multiple _test
...
package maintainer can place default values deviating from upstream defaults.
...
detailed blog post about the subject by the openQA development team.
...
You can read the Cleanup section for details.
...
the web UI configuration (which is the default). The updating is triggered when new tests are scheduled. For a periodic
