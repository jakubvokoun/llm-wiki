---
source_url: https://openbao.org/docs/concepts/seal/
fetched: 2026-07-03
---

# Seal/Unseal

When an OpenBao server is started, it starts in a *sealed* state. In this
...
*Unsealing* is the process of obtaining the plaintext root key necessary to
...
The data stored by OpenBao is encrypted. OpenBao needs the *encryption key* in order
to decrypt the data. The encryption key is also stored with the data
(in the *keyring*), but encrypted with another encryption key known as the *root key*.
...
![Shahir seals](/assets/images/openbao-shamir-seal-9ba848ed9f348e03b004fc0cd92af233.png)
...
This is the *unseal* process: the shares are added one at a time (in any
...
![Auto Unseal](/assets/images/openbao-auto-unseal-77f7440f411fd77071ed05032679d83d.png)
...
operations. When using Auto Unseal these operations require *recovery
keys* instead.
...
2. Perform step 1 for all the standby nodes, one at a time. It is necessary to
   ...
   quorum.
3. [Step down](/docs/commands/operator/step-down/) the
   ...
   elected.
4. The new active node will perform the migration. Monitor the server log in
...
5. Seal migration is now completed. Take down the old active node, update its
...
6. At this point, configuration files of all the nodes can be updated to only have the
