---
source_url: https://openbao.org/docs/configuration/listener/tcp/
fetched: 2026-07-03
---
...
specify [`api_addr`](/docs/configuration/#api_addr) and [`cluster_addr`](/docs/configuration/#cluster_addr) so OpenBao will
...
* `tls_cert_file` `(string: <required-if-enabled>, reloads-on-SIGHUP)` –
  ...
  combined file. On `SIGHUP`, the path set here *at OpenBao startup* will be used
...
* `tls_key_file` `(string: <required-if-enabled>, reloads-on-SIGHUP)` –
  ...
  configuration using `SIGHUP`. On `SIGHUP`, the path set here *at OpenBao
...
* `tls_cipher_suites` `(string: "")` – Specifies the list of supported
  ...
  is available in the [Golang TLS documentation](https://golang.org/src/crypto/tls/cipher_suites.go).
  ...
  **Note**: Go only consults the `tls_cipher_suites` list for TLSv1.2 and
  ...
  this and other TLS related changes, see the [Go TLS blog post](https://go.dev/blog/tls-cipher-suites).
* `tls_prefer_server_cipher_suites` `(string: "false")` – Specifies to prefer the
  ...
  **Warning**: The `tls_prefer_server_cipher_suites` parameter is deprecated.
  Setting it has no effect. See the above [Go blog post](https://go.dev/blog/tls-cipher-suites) for more
