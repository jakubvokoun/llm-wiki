...
def get_secret(secret_name):
    ...
    if not session_token:
        raise RuntimeError("Missing AWS_SESSION_TOKEN")
    headers = {"X-Aws-Parameters-Secrets-Token": session_token}
    request = urllib.request.Request(secrets_extension_endpoint, headers=headers, method="GET")
    with urllib.request.urlopen(request, timeout=3) as response:
        response_json = json.loads(response.read())
        return json.loads(response_json["SecretString"])
```
