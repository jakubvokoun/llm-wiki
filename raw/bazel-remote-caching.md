...
under your output user root (`<outputUserRoot>/cache/disk`), alongside the repository cache.
...
Generally, avoid modifying source files during a build.
...
because the outputs are different but they have the same action hash.
