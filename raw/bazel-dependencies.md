...
A target `A` *depends upon* a target `B` if `B` is needed by `A` at build or
...
A target `X` is *actually dependent* on target `Y` if `Y` must be present,
...
| ![Declared dependency graph with arrows connecting a, b, and c](/static/docs/images/a_b_c.svg)   **Declared** dependency graph | ![Actual dependency graph that matches the declared dependency                   graph with arrows connecting a, b, and c](/static/docs/images/a_b_c.svg)   **Actual** dependency graph |
...
direct *actual* dependency on `c`, but forgets to declare it in the build file
`a/BUILD`.

| `a / a.in` |  |
| --- | --- |
| ```         import b;         import c;         b.foo();         c.garply(); ``` |  |
| ![Declared dependency graph with arrows connecting a, b, and c](/static/docs/images/a_b_c.svg)   **Declared** dependency graph | ![Actual dependency graph with arrows connecting a, b, and c. An                   arrow now connects A to C as well. This does not match the                   declared dependency graph](/static/docs/images/a_b_c_ac.svg)   **Actual** dependency graph |
...
| ![Declared dependency graph with arrows connecting a and b.                   b no longer connects to c, which breaks a's connection to c](/static/docs/images/ab_c.svg)   **Declared** dependency graph | ![Actual dependency graph that shows a connecting to b and c,                   but b no longer connects to c](/static/docs/images/a_b_a_c.svg)   **Actual** dependency graph |
