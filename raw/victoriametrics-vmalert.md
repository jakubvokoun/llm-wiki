...
`vmalert` executes a list of the given
[alerting](/victoriametrics/vmalert/#alerting-rules)
or
[recording](/victoriametrics/vmalert/#recording-rules)
rules against configured `-datasource.url`. For sending alerting notifications
`vmalert` relies on [Alertmanager](https://github.com/prometheus/alertmanager)
configured via `-notifier.url` flag.
...
Configure `-vmalert.proxyURL` on VictoriaMetrics
[single-node](/victoriametrics/single-server-victoriametrics/#vmalert)
or
[vmselect in cluster version](/victoriametrics/cluster-victoriametrics/#vmalert)
...
* to proxy requests from [Grafana Alerting UI](https://grafana.com/docs/grafana/latest/alerting/)
  ;
* to access `vmalert`’s UI through
  [vmui](/victoriametrics/#vmui)
  .
...
please refer to the
[VictoriaMetrics Cloud documentation](/victoriametrics-cloud/alertmanager-setup-for-deployment/)
.

## Features [#](#features)

* Integration with [VictoriaMetrics](https://github.com/VictoriaMetrics/VictoriaMetrics)
  and
  [MetricsQL](/victoriametrics/metricsql/)
  ;
* Integration with
  [VictoriaLogs](/victorialogs/)
  and
  [LogsQL](/victorialogs/logsql/)
  . See
  [this doc](/victorialogs/vmalert/)
  ;
* Integration with
  ...
  [LogsQL](/victorialogs/logsql/)
  . See
  [this doc](/victoriatraces/vmalert/)
  ;
* Prometheus [alerting rules definition format](https://prometheus.io/docs/prometheus/latest/configuration/alerting_rules/#defining-alerting-rules)
  support;
* Integration with [Alertmanager](https://github.com/prometheus/alertmanager)
  starting from [Alertmanager v0.16.0-alpha](https://github.com/prometheus/alertmanager/releases/tag/v0.16.0-alpha.0)
  ;
* Keeps the alerts
  [state on restarts](/victoriametrics/vmalert/#alerts-state-on-restarts)
  ;
* Graphite datasource can be used for alerting and recording rules. See
  [these docs](/victoriametrics/vmalert/#graphite)
  ;
* Recording and Alerting rules backfilling (aka `replay`). See
  [these docs](/victoriametrics/vmalert/#rules-backfilling)
  ;
* Lightweight and without extra dependencies.
* Supports
  [reusable templates](/victoriametrics/vmalert/#reusable-templates)
  for annotations;
...
* `vmalert` executes rules within a group sequentially, but persistence of execution results to remote
  ...
  recording rule is reused in the next one. See how to
  [chain groups](/victoriametrics/vmalert/#chaining-groups)
  .
...
* datasource address - reachable endpoint with [Prometheus HTTP API](https://prometheus.io/docs/prometheus/latest/querying/api/#http-api)
  support for running queries against;
* notifier address [optional] - reachable [Alert Manager](https://github.com/prometheus/alertmanager)
  ...
  [config file](/victoriametrics/vmalert/#notifier-configuration-file)
  .
* remote write address [optional] -
  [remote write](/victoriametrics/vmagent/#victoriametrics-remote-write-protocol)
...
See also
[stream aggregation](/victoriametrics/stream-aggregation/)
.

See the full list of configuration flags in
[configuration](/victoriametrics/vmalert/#configuration)
section.
...
in [testdata](https://github.com/VictoriaMetrics/VictoriaMetrics/blob/master/app/vmalert/config/testdata)
folder.
...
Explore how to integrate `vmalert` with
[VictoriaMetrics Anomaly Detection](/anomaly-detection/)
in the following
[guide](/anomaly-detection/guides/guide-vmanomaly-vmalert/)
.
...
or
[MetricsQL](/victoriametrics/metricsql/)
expression. `vmalert` will execute the configured
...
There are two types of Rules:
  ...
  if execution result is not empty ([Prometheus alerting rules docs](https://prometheus.io/docs/prometheus/latest/configuration/alerting_rules)
  ).
  -
...
It is allowed to use [Go templating](https://golang.org/pkg/text/template/)
in annotations and labels(with limited support) to format data, iterate over
...
Additionally, `vmalert` provides some extra templating functions listed in
[template functions](/victoriametrics/vmalert/#template-functions)
and
[reusable templates](/victoriametrics/vmalert/#reusable-templates)
.
...
`vmalert` provides the following template functions, which can be used during
[templating](/victoriametrics/vmalert/#templating)
:
...
* `query` - executes the
  [MetricsQL](/victoriametrics/metricsql/)
  query against `-datasource.url` and returns the query result.
...
* `queryEscape` - escapes the input string, so it can be safely put inside [query arg](https://en.wikipedia.org/wiki/Percent-encoding)
  part of URL.
...
The content of `-rule.templates` can be also
[hot reloaded](/victoriametrics/vmalert/#hot-config-reload)
.
...
* `-remoteWrite.url` - URL to VictoriaMetrics (Single) or vminsert (Cluster). `vmalert` will persist alerts state
  to the configured address in the form of
  [time series](/victoriametrics/keyconcepts/#time-series)
...
* `-remoteRead.url` - URL to VictoriaMetrics (Single) or vmselect (Cluster). `vmalert` will try to restore alerts state
  ...
  `vmalert` process starts, and only for the configured rules. Config
  [hot reload](/victoriametrics/vmalert/#hot-config-reload)
  doesn’t trigger
  state restore.
...
with `restored` label in
[web UI](/victoriametrics/vmalert/#web)
.

### Link to alert source [#](#link-to-alert-source)
...
vmalert
[web UI](/victoriametrics/vmalert/#web)
to show the alert status and its fields.

It is possible to override the link format. For example, to make the link to
[vmui](/victoriametrics/single-server-victoriametrics/#vmui)
...
The `-external.alert.source` cmd-line flag supports
[templating](/victoriametrics/vmalert/#templating)
...
The `datasource` name is set to `VictoriaLogs` if rule’s type
Available from [v1.117.0](/changelog/#v11170)
(`prometheus`, `vlogs` or `graphite`) is `vlogs`.
...
In addition to `source` link, some extra links could be added to alert’s
[annotations](/victoriametrics/vmalert/#alerting-rules)
...
[multiple tenants](/victoriametrics/cluster-victoriametrics/#multitenancy)
:
...
* To run a separate `vmalert` instance per each tenant.
  ...
  according to
  [these docs](/victoriametrics/cluster-victoriametrics/#url-format)
  .
...
* To use the
  [multitenant endpoint](/victoriametrics/cluster-victoriametrics/#multitenancy-via-labels)

  Available from [v1.104.0](/changelog/#v11040)
  of vminsert as
...
The multitenant endpoint in vmselect is less efficient than
[specifying tenants in URL](/victoriametrics/cluster-victoriametrics/#url-format)
.
...
* To specify the `tenant` parameter for each rule group when `-clusterMode` is enabled in the
  [enterprise version of vmalert](/victoriametrics/enterprise/)
  . In this mode, `-datasource.url`, `-remoteRead.url` and `-remoteWrite.url` must contain only the hostname without tenant information, such as `-datasource.url=http://vmselect:8481`, and `vmalert` will automatically append the specified tenant to the URLs for querying and writing:
...
if `-clusterMode` is enabled. These labels can be used during
[templating](/victoriametrics/vmalert/#templating)
,
...
at [release page](https://github.com/VictoriaMetrics/VictoriaMetrics/releases/latest)
and in `*-enterprise`
tags at [Docker Hub](https://hub.docker.com/r/victoriametrics/vmalert/tags)
and [Quay](https://quay.io/repository/victoriametrics/vmalert?tab=tags)
.

### Reading rules from object storage [#](#reading-rules-from-object-storage)

[Enterprise version](/victoriametrics/enterprise/)
of `vmalert` may read alerting and recording rules
from object storage:
...
The following
[command-line flags](/victoriametrics/vmalert/#flags)
can be used for fine-tuning access to S3 and GCS:
...
* `-remoteWrite.url` and `-remoteRead.url` are optional and are needed only if
  you have recording rules or want to store
  [alerts state](/victoriametrics/vmalert/#alerts-state-on-restarts)
  on `vmalert` restarts;
...
[![vmalert single](/victoriametrics/vmalert_single.webp)](/victoriametrics/vmalert_single.webp)

#### Cluster VictoriaMetrics [#](#cluster-victoriametrics)

In
[cluster mode](/victoriametrics/cluster-victoriametrics/)
...
[![vmalert cluster](/victoriametrics/vmalert_cluster.webp)](/victoriametrics/vmalert_cluster.webp)

In case when you want to spread the load on these components - add balancers before them and configure
`vmalert` with balancer addresses. Please, see more about
[VictoriaMetrics cluster architecture](/victoriametrics/cluster-victoriametrics/#architecture-overview)
.

#### HA vmalert [#](#ha-vmalert)
...
[![vmalert ha](/victoriametrics/vmalert_ha.webp)](/victoriametrics/vmalert_ha.webp)

To avoid recording rules results and alerts state duplication in VictoriaMetrics server
don’t forget to configure
[deduplication](/victoriametrics/single-server-victoriametrics/#deduplication)
.
...
Check how to replace it with
[cluster VictoriaMetrics](/victoriametrics/vmalert/#cluster-victoriametrics)
if needed.

#### Downsampling and aggregation via vmalert [#](#downsampling-and-aggregation-via-vmalert)
...
via
[recording rules](/victoriametrics/vmalert/#recording-rules)
and backfill results to the configured `-remoteWrite.url`.
...
or as
[group’s](/victoriametrics/vmalert/#groups)
`interval` param). The interval helps to control “resolution” of the produced series.
...
[![vmalert multi cluster](/victoriametrics/vmalert_multicluster.webp)](/victoriametrics/vmalert_multicluster.webp)

Please note,
[replay](/victoriametrics/vmalert/#rules-backfilling)
feature may be used for transforming historical data.
...
[stream aggregation](/victoriametrics/stream-aggregation/)
and
[downsampling](/victoriametrics/single-server-victoriametrics/#downsampling)
.

#### Multiple remote writes [#](#multiple-remote-writes)
...
[![vmalert multiple remote write destinations](/victoriametrics/vmalert_multiple_rw.webp)](/victoriametrics/vmalert_multiple_rw.webp)
...
[data persisting when storage is unreachable](/victoriametrics/vmagent/#replication-and-high-availability)
,
or time series modification via
[relabeling](/victoriametrics/relabeling/)
.
...
`vmalert` web UI can be accessed from
[single-node version of VictoriaMetrics](/victoriametrics/single-server-victoriametrics/)
and from
[cluster version of VictoriaMetrics](/victoriametrics/cluster-victoriametrics/)
.
...
implements [Graphite Render API](https://graphite.readthedocs.io/en/stable/render_api.html)
for `format=json`.
...
vmalert supports
[VictoriaLogs](/victorialogs/)
as a datasource for writing alerting and recording rules using
[LogsQL](/victorialogs/logsql/)
. See
[this doc](/victorialogs/vmalert/)
for details.
...
vmalert supports
[VictoriaTraces](/victoriatraces/)
as a (`vlogs`) datasource for writing alerting and recording rules using
[LogsQL](/victorialogs/logsql/)
. See
[this doc](/victoriatraces/vmalert/)
for details.

## Rules backfilling [#](#rules-backfilling)
...
as described in
[backfilling docs](/victoriametrics/single-server-victoriametrics/#backfilling)
.
...
vmalert sends rule’s expression to
[/query\_range](/victoriametrics/keyconcepts/#range-query)
endpoint
of the configured `-datasource.url`. Returned data is then processed according to the rule type and
backfilled to `-remoteWrite.url` via
[remote Write protocol](/victoriametrics/vmagent/#victoriametrics-remote-write-protocol)
.
...
#### Recording rules [#](#recording-rules-1)

The result of recording rules `replay` should match with results of normal rules evaluation.

#### Alerting rules [#](#alerting-rules-1)

The result of alerting rules `replay` is time series reflecting
[alert’s state](/victoriametrics/vmalert/#alerts-state-on-restarts)
.
...
.
See more details in
[vmalert-tool](/victoriametrics/vmalert-tool/#unit-testing-for-rules)
.

## Monitoring [#](#monitoring)

`vmalert` exports various metrics in Prometheus exposition format at `http://vmalert-host:8880/metrics` page.
See the [list of recommended alerting rules](https://github.com/VictoriaMetrics/VictoriaMetrics/blob/master/deployment/docker/rules/alerts-vmalert.yml)
to track the vmalert health.
We recommend setting up regular scraping of this page either through
[vmagent](/victoriametrics/vmagent/)
or by Prometheus-compatible scraper,
...
See [these docs](https://cloud.google.com/stackdriver/docs/managed-prometheus/troubleshooting#missing-metric-type)
for details.

Use the official [Grafana dashboard](https://grafana.com/grafana/dashboards/14950)
for `vmalert` overview.
...
1. Always set
   [group’s interval](/victoriametrics/vmalert/#groups)
   to be **equal to or greater than**
   the
   [time series resolution](/victoriametrics/keyconcepts/#time-series-resolution)
   .
2. Don’t set labels with dynamic values to `labels`
   [param](/victoriametrics/vmalert/#alerting-rules)
   .
   * Example: setting `label: {{$value}}` to the rule will break its
     [alert state tracking](/victoriametrics/vmalert/#alert-state)
...
3. vmalert runs
   [instant queries](/victoriametrics/keyconcepts/#instant-query)
   ...
   If
   [series resolution](/victoriametrics/keyconcepts/#time-series-resolution)
...
5. Don’t skip `[lookbehind-window]` in rollup functions.
   * Example: `rate(errors_total) > 0`. MetricsQL
     [allows omitting lookbehind window](/victoriametrics/metricsql/#metricsql-features)
     but that works well only with
     [/api/v1/query\_range](/victoriametrics/keyconcepts/#range-query)
     .
     For
     [instant requests](/victoriametrics/keyconcepts/#instant-query)
     setting window
     makes the query predictable.
6. Make sure the `[lookbehind-window]` in your expression is at least **2× larger** than
   [time series resolution](/victoriametrics/keyconcepts/#time-series-resolution)
   .
...
8. Don’t skip step in
   [subqueries](/victoriametrics/metricsql/#subqueries)
   .
...
vmalert keeps the last `-rule.updateEntriesLimit` updates (or `update_entries_limit`
[per-rule config](/victoriametrics/vmalert/#alerting-rules)
)
for each rule. You can see these updates in vmalert’s
[web UI](/victoriametrics/vmalert/#web)
:
...
[![vmalert state](/victoriametrics/vmalert_state.webp)](/victoriametrics/vmalert_state.webp)
...
4. `Series fetched` - the number of series scanned during execution. See
   [never-firing alerts](/victoriametrics/vmalert/#never-firing-alerts)
   .
5. `Duration` - how long it took to evaluate the rule.
   ...
   * See how to handle
     [slow queries](/victoriametrics/troubleshooting/#slow-queries)
     .
6. `cURL` - a sample HTTP request that vmalert sent to `-datasource.url` during evaluation.
   ...
   * *Sensitive data is removed from the `curl` example – see the
     [security](/victoriametrics/vmalert/#security)
...
See more about
[data delay](/victoriametrics/vmalert/#data-delay)
.
...
*See more about alerting rules in
[Monitoring](/victoriametrics/vmalert/#monitoring)
.*
...
[vmui](/victoriametrics/single-server-victoriametrics/#vmui)
or Grafana to track how
alerts state changed in time. See [query statistics dashboard](https://github.com/VictoriaMetrics/VictoriaMetrics/blob/master/dashboards/alert-statistics.json)
as example for tracking historical alerts state.
...
[![vmalert expected evaluation](/victoriametrics/vmalert_ts_normal.gif)](/victoriametrics/vmalert_ts_normal.gif)
...
[![vmalert evaluation when data is delayed](/victoriametrics/vmalert_ts_data_delay.gif)](/victoriametrics/vmalert_ts_data_delay.gif)

Please note, data delay is inevitable in distributed systems. And it is better to account for it rather than ignore it.

By default, recently written samples to VictoriaMetrics
[aren’t visible for queries](/victoriametrics/keyconcepts/#query-latency)
...
See
[common mistakes](/victoriametrics/vmalert/#common-mistakes)
for recommendations for dealing with sporadic or delayed data.

### Debug mode [#](#debug-mode)

vmalert allows configuring more detailed logging for specific rule starting from
Available from [v1.116.0](/changelog/#v11160)
.
Or for all rules within the
[group](/victoriametrics/vmalert/#groups)
...
Sensitive info is stripped from the `curl` examples - see
[security](/victoriametrics/vmalert/#security)
section for more details.

### Flapping alerts [#](#flapping-alerts)
...
4. Make sure your rule can handle possible
   [data delays](/victoriametrics/vmalert/#data-delay)
   from the datasource.
...
vmalert can detect
Available from [v1.91.0](/changelog/#v1910)
if alert’s expression doesn’t match any time series in runtime.
...
See more details in this [GitHub Issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/4039)
and
read [Never-firing alerts](https://victoriametrics.com/blog/never-firing-alerts/)
blogpost.

### Series with the same labelset [#](#series-with-the-same-labelset)
...
The error means there is a collision between
[time series](/victoriametrics/keyconcepts/#time-series)
during evaluation.
...
To fix this, use
[label\_replace](/victoriametrics/metricsql/#label_replace)
to preserve the distinct labelset.
...
[Enterprise version of vmagent](/victoriametrics/enterprise/)
supports the ability to accept [mTLS](https://en.wikipedia.org/wiki/Mutual_authentication)
...
By default system-wide [TLS Root CA](https://en.wikipedia.org/wiki/Root_certificate)
...
See general recommendations regarding
[security](/victoriametrics/single-server-victoriametrics/#security)
.

vmalert
[web UI](/victoriametrics/vmalert/#web)
, logs, and exported metrics contain details such as
[group](/victoriametrics/vmalert/#groups)
configurations, active alerts,
[alerts state](/victoriametrics/vmalert/#alerts-state-on-restarts)
,
[notifiers configuration](/victoriametrics/vmalert/#notifier-configuration-file)
. Consider limiting user’s access
...
* Log messages, the UI, and exported metrics contain full path to the configured rule files. These file paths can be stripped
  by enabling `-rule.stripFilePath` command-line flag
  Available from [v1.143.0](/changelog/#v11430)
  ;
...
[Alerts state](/victoriametrics/vmalert/#alerts-state-on-restarts)
page or
[debug mode](/victoriametrics/vmalert/#debug-mode)
could emit additional information about configured
...
See also
[mTLS protection docs](/victoriametrics/vmalert/#mtls-protection)
.
...
#### Enterprise flags [#](#enterprise-flags)

These flags are available only in
[VictoriaMetrics enterprise](/victoriametrics/enterprise/)
.
...
To set additional URL params for specific
[group of rules](/victoriametrics/vmalert/#groups)
modify
the `params` group:
...
Available from [v1.113.0](/changelog/#v11130)
.
...
### Notifier configuration file [#](#notifier-configuration-file)
...
The list of configured or discovered Notifiers can be explored via
[UI](/victoriametrics/vmalert/#web)
.
