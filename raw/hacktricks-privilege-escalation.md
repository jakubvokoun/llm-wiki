...
You can find a good vulnerable kernel list and some already **compiled exploits** here: <https://github.com/lucyoa/kernel-exploits> and [exploitdb sploits](https://gitlab.com/exploit-database/exploitdb-bin-sploits).
...
[Adreno A7xx Sds Rb Priv Bypass Gpu Smmu Kernel Rw](../../binary-exploitation/linux-kernel-exploitation/adreno-a7xx-sds-rb-priv-bypass-gpu-smmu-kernel-rw.html)
[Arm64 Static Linear Map Kaslr Bypass](../../binary-exploitation/linux-kernel-exploitation/arm64-static-linear-map-kaslr-bypass.html)
...
### [Sudo host-based rules bypass (CVE-2025-32462)](#sudo-host-based-rules-bypass-cve-2025-32462)
...
### [Dmesg signature verification failed](#dmesg-signature-verification-failed)
...
### [More system enumeration](#more-system-enumeration)
...
## [Enumerate possible defenses](#enumerate-possible-defenses)
...
[Container Security](container-security/index.html)
...
### [Vulnerable Software Installed](#vulnerable-software-installed)
...
> [!NOTE] > *Note that these commands will show a lot of information that will mostly be useless, therefore it’s recommended some applications like OpenVAS or similar that will check if any installed software version is vulnerable to known exploits*
...
Always check for possible [**electron/cef/chromium debuggers** running, you could abuse it to escalate privileges](electron-cef-chromium-debugger-abuse.html). **Linpeas** detect those by checking the `--inspect` parameter inside the command line of the process.
Also **check your privileges over the processes binaries**, maybe you can overwrite someone.

### [Cross-user parent-child chains](#cross-user-parent-child-chains)
...
### [Deleted executables and deleted-open files](#deleted-executables-and-deleted-open-files)
...
#### [/proc/$pid/maps & /proc/$pid/mem](#procpidmaps--procpidmem)
...
### [ProcDump for linux](#procdump-for-linux)
...
### [Credentials from Process Memory](#credentials-from-process-memory)
...
#### [Search Regexes/[truffleproc](https://github.com/controlplaneio/truffleproc)](#search-regexestruffleproc)
...
### [Crontab UI (alseambusher) running as root – web-based scheduler privesc](#crontab-ui-alseambusher-running-as-root--web-based-scheduler-privesc)
...
[Wildcards Spare tricks](wildcards-spare-tricks.html)

### [Bash arithmetic expansion injection in cron log parsers](#bash-arithmetic-expansion-injection-in-cron-log-parsers)
...
### [Cron script overwriting and symlink](#cron-script-overwriting-and-symlink)
...
### [Symlink validation and safer file handling](#symlink-validation-and-safer-file-handling)
...
### [Custom-signed cron binaries with writable payloads](#custom-signed-cron-binaries-with-writable-payloads)
...
### [Root backups that preserve attacker-set mode bits (pg\_basebackup)](#root-backups-that-preserve-attacker-set-mode-bits-pg_basebackup)
...
### [Invisible cron jobs](#invisible-cron-jobs)
...
### [Writable *.service* files](#writable-service-files)
...
### [Writable service binaries](#writable-service-binaries)
...
### [systemd PATH - Relative Paths](#systemd-path---relative-paths)
...
### [Socket activation + writable unit path (create missing service)](#socket-activation--writable-unit-path-create-missing-service)
...
[Socket Command Injection](socket-command-injection.html)
...
### [Writable Docker Socket](#writable-docker-socket)

The Docker socket, often found at `/var/run/docker.sock`, is a critical file that should be secured. By default, it’s writable by the `root` user and members of the `docker` group. Possessing write access to this socket can lead to privilege escalation. Here’s a breakdown of how this can be done and alternative methods if the Docker CLI isn’t available.

#### [**Privilege Escalation with Docker CLI**](#privilege-escalation-with-docker-cli)
...
#### [**Using Docker API Directly**](#using-docker-api-directly)
...
Note that if you have write permissions over the docker socket because you are **inside the group `docker`** you have [**more ways to escalate privileges**](interesting-groups-linux-pe/index.html#docker-group). If the [**docker API is listening in a port** you can also be able to compromise it](../../network-services-pentesting/2375-pentesting-docker.html#compromising).

Check **more ways to break out from containers or abuse container runtimes to escalate privileges** in:

[Container Security](container-security/index.html)

## [Containerd (ctr) privilege escalation](#containerd-ctr-privilege-escalation)

If you find that you can use the **`ctr`** command read the following page as **you may be able to abuse it to escalate privileges**:

[Containerd (ctr) Privilege Escalation](containerd-ctr-privilege-escalation.html)

## [**RunC** privilege escalation](#runc-privilege-escalation)

If you find that you can use the **`runc`** command read the following page as **you may be able to abuse it to escalate privileges**:

[RunC Privilege Escalation](runc-privilege-escalation.html)
...
[D-Bus Enumeration & Command Injection Privilege Escalation](d-bus-enumeration-and-command-injection-privilege-escalation.html)
...
### [Outbound filtering quick triage](#outbound-filtering-quick-triage)
...
### [Local-only service triage workflow](#local-only-service-triage-workflow)
...
### [LinPEAS as a network scanner (network-only mode)](#linpeas-as-a-network-scanner-network-only-mode)
...
### [Generic Enumeration](#generic-enumeration-1)
...
[Interesting Groups - Linux Privesc](interesting-groups-linux-pe/index.html)
...
### [Known passwords](#known-passwords)
...
## [Writable PATH abuses](#writable-path-abuses)
...
### [Writable `__pycache__` / `.pyc` poisoning in sudo-allowed Python imports](#writable-__pycache__--pyc-poisoning-in-sudo-allowed-python-imports)
...
### [BASH\_ENV preserved via sudo env\_keep → root shell](#bash_env-preserved-via-sudo-env_keep--root-shell)
...
### [Terraform via sudo with preserved HOME (!env\_reset)](#terraform-via-sudo-with-preserved-home-env_reset)
...
### [TF\_VAR overrides + symlink validation bypass](#tf_var-overrides--symlink-validation-bypass)
...
### [requiretty / !requiretty](#requiretty--requiretty)
...
### [Sudo env\_keep+=PATH / insecure secure\_path → PATH hijack](#sudo-env_keeppath--insecure-secure_path--path-hijack)
...
### [Sudo command/SUID binary without command path](#sudo-commandsuid-binary-without-command-path)
...
[Payload examples to execute.](payloads-to-execute.html)

### [SUID binary with command path](#suid-binary-with-command-path)
...
### [Writable script executed by a SUID wrapper](#writable-script-executed-by-a-suid-wrapper)
...
### [SUID Binary – .so injection](#suid-binary--so-injection)
...
[GTFOArgs: Argument Injection Exploitation Vector List | GTFOArgs is a curated list of Unix binaries whose arguments can be exploited for argument injection, enabling shell escapes, privilege escalation, file read/write, and other post-exploitation techniques.](https://gtfoargs.github.io/)

### [FallOfSudo](#fallofsudo)

If you can access `sudo -l` you can use the tool [**FallOfSudo**](https://github.com/CyberOne-Security/FallofSudo) to check if it finds how to exploit any sudo rule.

### [Reusing Sudo Tokens](#reusing-sudo-tokens)
...
### [/var/run/sudo/ts/<Username>](#varrunsudotsusername)
...
### [/etc/sudoers, /etc/sudoers.d](#etcsudoers-etcsudoersd)
...
[ld.so privesc exploit example](ld.so.conf-example.html)
...
[Linux Capabilities](linux-capabilities.html)
...
### [Hidden ACL backdoor on sudoers drop-ins](#hidden-acl-backdoor-on-sudoers-drop-ins)
...
## [Open shell sessions](#open-shell-sessions)
...
### [screen sessions hijacking](#screen-sessions-hijacking)
...
### [Debian OpenSSL Predictable PRNG - CVE-2008-0166](#debian-openssl-predictable-prng---cve-2008-0166)
...
### [SSH Interesting configuration values](#ssh-interesting-configuration-values)

* **PasswordAuthentication:** Specifies whether password authentication is allowed. The default is `no`.
* **PubkeyAuthentication:** Specifies whether public key authentication is allowed. The default is `yes`.
* **PermitEmptyPasswords**: When password authentication is allowed, it specifies whether the server allows login to accounts with empty password strings. The default is `no`.

### [Login control files](#login-control-files)
...
### [ForwardAgent/AllowAgentForwarding](#forwardagentallowagentforwarding)
...
[SSH Forward Agent exploitation](ssh-forward-agent-exploitation.html)
...
### [Passwd/Shadow Files](#passwdshadow-files)
...
### [Weird Location/Owned files](#weird-locationowned-files)
...
### [Modified files in last mins](#modified-files-in-last-mins)
...
### [\*\_history, .sudo\_as\_admin\_successful, profile, bashrc, httpd.conf, .plan, .htpasswd, .git-credentials, .rhosts, hosts.equiv, Dockerfile, docker-compose.yml files](#_history-sudo_as_admin_successful-profile-bashrc-httpdconf-plan-htpasswd-git-credentials-rhosts-hostsequiv-dockerfile-docker-composeyml-files)
...
### [**Script/Binaries in PATH**](#scriptbinaries-in-path)
...
### [Generic Creds Search/Regex](#generic-creds-searchregex)
...
### [Logrotate exploitation](#logrotate-exploitation)
...
### [/etc/sysconfig/network-scripts/ (Centos/Redhat)](#etcsysconfignetwork-scripts-centosredhat)
...
### [**init, init.d, systemd, and rc.d**](#init-initd-systemd-and-rcd)
...
[NFS no\_root\_squash/no\_all\_squash misconfiguration PE](nfs-no_root_squash-misconfiguration-pe.html)

### [Escaping from restricted Shells](#escaping-from-restricted-shells)

[Escaping from Jails](escaping-from-limited-bash.html)

### [Cisco - vmanage](#cisco---vmanage)

[Cisco - vmanage](cisco-vmanage.html)

## [Android rooting frameworks: manager-channel abuse](#android-rooting-frameworks-manager-channel-abuse)
...
[Android Rooting Frameworks Manager Auth Bypass Syscall Hook](android-rooting-frameworks-manager-auth-bypass-syscall-hook.html)

## [VMware Tools service discovery LPE (CWE-426) via regex-based exec (CVE-2025-41244)](#vmware-tools-service-discovery-lpe-cwe-426-via-regex-based-exec-cve-2025-41244)
...
[Vmware Tools Service Discovery Untrusted Search Path Cve 2025 41244](vmware-tools-service-discovery-untrusted-search-path-cve-2025-41244.html)
...
## [Linux/Unix Privesc Tools](#linuxunix-privesc-tools)

### [**Best tool to look for Linux local privilege escalation vectors:** [**LinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/linPEAS)](#best-tool-to-look-for-linux-local-privilege-escalation-vectors-linpeas)
...
**Mestaploit:** ***multi/recon/local\_exploit\_suggester***
...
* [0xdf – HTB Era: forged .text\_sig payload for cron-executed monitor](https://0xdf.gitlab.io/2025/11/29/htb-era.html)
* [0xdf – Holiday Hack Challenge 2025: Neighborhood Watch Bypass (sudo env\_keep PATH hijack)](https://0xdf.gitlab.io/holidayhack2025/act1/neighborhood-watch)
...
* [0xdf – HTB Previous (sudo terraform dev\_overrides + TF\_VAR symlink privesc)](https://0xdf.gitlab.io/2026/01/10/htb-previous.html)
* [0xdf – HTB Slonik (pg\_basebackup cron copy → SUID bash)](https://0xdf.gitlab.io/2026/02/12/htb-slonik.html)
