...
**Categories:** Open Source Tech, Monitoring, Time Series Database
...
A rule is an expression that is periodically evaluated against a datasource at a specified interval. The expression is written in a query language (like PromQL, MetricsQL, or LogsQL) and defines what data to retrieve and analyze.
...
Alert transitions before reaching Alertmanager
...
Alertmanager receives resolved status update
...
Earlier, we mentioned that the system runs rules at set intervals, but what actually decides that timing? That would be the *group*.
...
Group wait, interval, and repeat timing

**Note: The difference between 'group_interval' and 'repeat_interval'**
...
* `repeat_interval` works a bit differently. It ensures that even if nothing changes, the alert is resent every 4 hours, *from the last time it was sent*.
...
Alerts grouped by environment and cluster
