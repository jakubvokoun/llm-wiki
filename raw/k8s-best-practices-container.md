...
Before designing any solution, it's important to review all the basic and important points: scale, maintenance, architecture ... **infrastructure**. To build a service that will run in a container, you need to keep in mind a basic and extremely important point:
...
* **Always**: The agent searches the image locally based on the same digest, if it doesn't exist, the agent searches externally. (**Preferred for production**)
...
In productive environments, always keep tags explicit, this prevents image versions with unwanted updates or unexpected behavior from happening.
...
Separating the build into stages helps with readability and reduces the final image size, as only the last stage matters and the rest is discarded.
...
In Kubernetes, prefer using `livenessProbe` and `readinessProbe` over Docker's HEALTHCHECK, as they provide more control and integration with the orchestrator.
