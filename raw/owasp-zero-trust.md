...
Every connection must be encrypted and authenticated, whether it's between your office and the cloud, between internal systems, or from home to work. Network location doesn't determine trust level. Use strong encryption (TLS 1.3 or better) for everything.

### 3. Access to Resources is Granted on a Per-Session Basis

Don't give permanent access to anything. Each time someone tries to access a resource, evaluate whether they should be allowed. Sessions should be short-lived and require re-authentication when they expire. No "set it and forget it" access.
...
You need MFA for everyone - employees, contractors, and partners. Here's what works best:
...
Set up conditional access so high-risk situations require stronger authentication. OMB M-22-09 mandates phishing-resistant MFA for U.S. federal agencies.
...
Follow comprehensive authentication best practices when implementing user account management.
...
* **API security gateways**: Authenticate every API call, validate request schemas, and enforce rate limits for microservices communication
...
* **Use encryption**: Protect data whether it's stored or moving with proper cryptographic storage
...
These phases line up with the CISA Zero Trust Maturity Model v2.0, but your timeline will depend on your organization's size and resources.
