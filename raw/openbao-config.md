---
source_url: https://openbao.org/docs/configuration/
fetched: 2026-07-03
---
...
* `audit` `([Audit](/docs/configuration/audit/): <none>)` – Specifies [audit device
  configuration](/docs/configuration/audit/). Can be specified multiple times.
* `storage` `([StorageBackend](/docs/configuration/storage/): <required>)`  –
  Configures the storage backend where OpenBao data is stored. Please see the
  [storage backends documentation](/docs/configuration/storage/) for the full list of
  ...
  options.
* `ha_storage` `([StorageBackend](/docs/configuration/storage/): nil)`  – Configures
...
* `listener` `([Listener](/docs/configuration/listener/): <required>)` – Configures how
  OpenBao is listening for API requests.
* `user_lockout` `([UserLockout](/docs/configuration/user-lockout/): nil)`. –
...
* `seal` `([Seal](/docs/configuration/seal/): nil)` – Configures the seal type to use for
  auto-unsealing.
...
* `initialize` `([Initialize](/docs/configuration/self-init/): <none>)` – Specifies one-time
  [declarative self-initialization](/docs/configuration/self-init/). Can be specified multiple times.
...
* `plugin` `([Plugin](/docs/configuration/plugins/): <none>)` – Specifies declarative plugin
...
* `plugin_auto_download` `bool: false` - Whether automatic plugin downloading is enabled. See
  [OCI plugin documentation](/docs/configuration/plugins/) for more information.
...
* `plugin_download_behavior` `string: "fail"` - Whether plugin downloading is treated as fatal
  failure. See [OCI plugin documentation](/docs/configuration/plugins/) for more information.
* `telemetry` `([Telemetry](/docs/configuration/telemetry/): <none>)` – Specifies the telemetry
  reporting system.
...
* `max_lease_ttl` `(string: "768h")`  – Specifies the maximum possible lease
  ...
  [auth](/docs/commands/auth/tune/#_max_lease_ttl) or
  [secret](/docs/commands/secrets/tune/#_max_lease_ttl) commands.
...
* `log_level` `(string: "info")` - Log verbosity level.
  ...
  Note: On SIGHUP (`sudo kill -s HUP` *pid of bao*), if a valid value is
...
The following parameters are used on backends that support [High Availability](/docs/concepts/ha/).
...
* `api_addr` `(string: "")`  – Specifies the address (full URL) to advertise to
  ...
  used for [plugin backends](/docs/plugins/). This can also be provided via the
  environment variable `VAULT_API_ADDR`. In general this should be set as a full
  URL that points to the value of the [`listener`](/docs/configuration/listener/) address.
...
* `disable_clustering` `(bool: false)` – Specifies whether clustering features
  ...
  will disable these features *only when that node is the active node*. This
