...
Attack is described in detail at: https://owasp.org/www-community/attacks/Reverse_Tabnabbing
...
* Add the HTTP response header `Referrer-Policy: no-referrer` to every HTTP response sent by the application. This configuration will ensure that no referrer information is sent along with requests from the page.
