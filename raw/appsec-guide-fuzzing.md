---
source_url: https://appsec.guide/docs/fuzzing/
fetched: 2026-06-19
---
...
Each language section concludes with a language-specific "Techniques" subsection. If a technique applies to all languages and fuzzers, then it is listed in the Techniques section at the very end of the chapter. This very last section also contains an FAQ and information about Fuzzing environments.
...
Fuzzing is no silver bullet, just like unit tests or static analysis aren't.
...
```
 1fn fuzz(corpus) {
 2  let bug_set = []; // We start with an empty set
 3  while let Some(test_case) = schedule(corpus) { // Now, we pick one test case after the other
 4    let offspring = mutate(test_case); // Each test case is mutated to create offspring
 5    let observations = execute(offspring); // New test cases are executed
 6
 7    if (is_interesting(observations)) {
 8      corpus.append(offspring); // If the observations are interesting we keep the new test case
 9    }
10    if (is_bug(observations)) {
11      bug_set.append(offspring); // If the observations indicate that a bug occurred then we store the test case
12    }
13  }
14  return bug_set; // The set of bugs is returned
15}
```

**`schedule(test_case)`:** start the fuzzing loop by scheduling one test case after the other from the corpus. The `schedule` function eventually decides to stop the fuzzing campaign by returning nothing.

**`mutate(test_case)`:** mutate the test case to create offspring. If the test inputs are byte arrays, then common mutation strategies include flipping bits, inserting new bytes, or truncating.

**`execute(test_case)`:** execute the new test case to generate observations. The simplest observation is execution time; more complex observations include covered lines of code within the SUT.

If something represents progress toward finding bugs, then we add it to the corpus. Traditionally, the interestingness of a test case coincides with increases in code coverage.

## Introduction to fuzzers

Every fuzzing setup consists of an instrumented System Under Test (SUT), the fuzzing harness, and the fuzzer runtime. A runtime for the instrumentation may also be required. For example, the optional AddressSanitizer (ASan) instrumentation adds a runtime that is used to detect [memory corruption bugs](https://en.wikipedia.org/wiki/Memory_corruption) like heap-buffer overflows more reliably.
...
**SUT (System Under Test):** This is the code you want to test. To create a fuzzing build of your SUT, you need to control how the application's code is compiled and linked.
...
**Instrumentation runtime:** Instrumentations like AddressSanitizer or UndefinedBehaviorSanitizer come with a runtime. A fuzzer must be compatible with the sanitizer for bugs to be detected reliably. In memory-safe languages like Go or Rust you are less likely to need sanitizers. There is also a different class of instrumentations (e.g., SanitizerCoverage) that provides feedback to the fuzzer during execution.
