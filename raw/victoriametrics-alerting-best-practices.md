...
By Roman Khavronenko and Mathias Palmersheim  
...
We heavily use templating in annotations to enrich each unique alerting notification with personalized information. It's OK to use templates like `$value` or `$labels` in annotations, as annotations aren't taken into account during `for` checks.
...
During alerting rules evaluation, vmalert persists alerts state changes in the form of time series with names **ALERTS** and **ALERTS_FOR_STATE**. Using these metrics we can see the history of alerts state changes. For this purpose, we have built a Grafana dashboard for alerts statistics.
...
VictoriaMetrics ecosystem provides all required tools for defining, testing and monitoring alerting processes. Key resources include:
...
4. VictoriaMetrics vmalert documentation
5. VictoriaMetrics vmalert-tool documentation
